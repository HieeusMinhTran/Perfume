...
class CartController extends Controller
{
    ...
    public function add_to_cart(Request $request){
        ...
        $cart->idProduct = (int) $data['idProduct'];
        $cart->QuantityBuy = (int) $data['QuantityBuy'];
        $cart->PriceNew = (int) $data['PriceNew'];
        $cart->Total = (int) $data['PriceNew']*$data['QuantityBuy'];
        $cart->idCustomer = Session::get('idCustomer');
        $cart->AttributeProduct = $data['AttributeProduct'];
        $cart->idProAttr = $data['idProAttr'];
        $qty_of_attr = $data['qty_of_attr'];
...
{
    $this->checkCart();
    Cart::destroy((int)$idCart); // Ép kiểu idCart thành integer
}
    ...
    public function submit_payment(Request $request){
        ...
        if($data['checkout'] == 'vnpay'){
            ...
            $vnp_Returnurl = "http://localhost:8080/perfumealtelier/success-order";
            ...
            if (!isset($data['address_rdo'])) {
                return response()->json(['error' => 'Address not selected'], 400);
            }
